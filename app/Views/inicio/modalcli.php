...
// function BuscarNomenclatura()
// {
//     include '../includes/BD_con/db_con.php';
//     mysqli_set_charset($conexion, 'utf8');
//     $consulta2 = mysqli_query($conexion, "SELECT id, ccodcta, cdescrip FROM ctb_nomenclatura");
...
//             <td scope="row"> <button type="button" class="btn btn-success" onclick= "seleccionar_cuenta_ctb(`#id_modal_hidden`,[`' . $id . '`,`' . $ccodcta . '`,`' . $cdescrip . '`])" >Aceptar</button> </td>
//             </tr> ';
//     }
// }
                    ...
                    <table id="tabla_m_cliente" class="table table-striped table-hover" style="width: 100% !important;">
                        ...
                        <?php foreach ($datos as $dato) { ?>
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-success btn-sm" onclick="seleccionar_cliente('id_cliente,nombrecli,nomcom,telefono,departamento/A,A,A,A,A'+'/'+'/#/#',['<?= $dato['id']; ?>','<?= $dato['nombre']; ?>','<?= $dato['nomcom']; ?>','<?= $dato['telefono']; ?>','<?= $dato['departamento']; ?>'])"><i class="fa-solid fa-check me-2"></i>Aceptar</button>
                                </td>
                            </tr>
                        <?php }; ?>
                        </tbody>
